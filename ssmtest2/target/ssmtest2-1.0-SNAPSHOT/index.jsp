...
<script type="text/javascript">
    ...
    function build_emps_table(result) {
        ...
        // alert("ssss")
        //然后添加数据
        var emps = result.exttend.pageInfo.list;
        // alert(emps);
    ...
    function build_page_info(result) {
        ...
        //然后添加数据
        $("#page_info_area").append("当前第 " + result.exttend.pageInfo.pageNum + " 页，总共 "
            + result.exttend.pageInfo.pages + " 页，总计 " + result.exttend.pageInfo.total + " 条记录");
        totalPages = result.exttend.pageInfo.pages;
        currentPage = result.exttend.pageInfo.pageNum;
    }
    ...
    function build_page_nav(result) {
        ...
        //如果已经是首页，则禁用首页和前一页
        if (result.exttend.pageInfo.hasPreviousPage == false) {
            ...
            prePageLi.click(function () {
                to_page(result.exttend.pageInfo.pageNum - 1)
            });
        }
        ul.append(firstPageLi).append(prePageLi);
        //添加页码提示
        $.each(result.exttend.pageInfo.navigatepageNums, function (index, item) {
            let numLi = $("<li></li>").append($("<a></a>").append(item));
            if (result.exttend.pageInfo.pageNum == item) {
        ...
        //如果已经是末页，则禁用下一页和末页
        if (result.exttend.pageInfo.hasNextPage == false) {
            ...
            lastPageLi.click(function () {
                to_page(result.exttend.pageInfo.pages);
            });
        }
    ...
    function getDepts(ele) {
        ...
        $.ajax({
            ...
            type: "GET",
            success: function (result) {
                $.each(result.exttend.depts, function () {
    ...
    function validate_add_form() {
        ...
        if (!regEmail.test(email)) {
            ...
            show_validate_msg("#email_add_input", "success", "邮箱格式准确");
        }
        return true;
    }
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.code == 100) {
                    ...
                    show_validate_msg("#empName_add_input", "error", result.exttend.va_msg);
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.code == 100) {
                    ...
                    if (undefined != result.exttend.errorFields.email) {
                        //显示邮箱错误信息
                        show_validate_msg("#email_add_input", "error", result.exttend.errorFields.email);
                    }
                    if (undefined != result.exttend.errorFields.empName) {
                        //显示员工名字的错误信息
                        show_validate_msg("#empName_add_input", "error", result.exttend.errorFields.empName);
                    }
                }
            }
        });
    ...
    function getEmp(id) {
        $.ajax({
            ...
            type: "GET",
            success: function (result) {
                let empData = result.exttend.emp;
